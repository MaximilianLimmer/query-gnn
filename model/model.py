...
class HybridQueryGNN(torch.nn.Module):
    def __init__(self, input_dim=27): # Full input size from preprocessing
        super().__init__()
        # The GNN sees (input_dim - 1) because we hide the Total Cost column
        gnn_input_dim = input_dim - 1

        self.conv1 = GATv2Conv(gnn_input_dim, 32, heads=2)
        ...
        self.final_head = torch.nn.Sequential(
            torch.nn.Linear(32 + 1, 16), # 32 from GNN + 1 from shortcut
            torch.nn.ReLU(),
    ...
    def forward(self, data):
        ...
        # 1. Structural Path: Hide 'Total Cost' (index 24)
        # Features: [0...23] + [25...26] = 26 features total
        ...
        # 2. Shortcut Path: Use the logged 'Total Cost' (index 24)
        # Since the root node usually represents the whole plan's cost
